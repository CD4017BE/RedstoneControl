...
public class GuiProcessor extends ModularGui {
	...
	public GuiProcessor(IProcessor tile, AdvancedContainer cont) {
		super(cont);
		...
		new Progressbar(comps, 36, 7, 88, 16, 186, 249, Progressbar.H_FILL, tile::getExhaustion, 0, 100).tooltip("gui.rs_ctr.processor.power");
		new Button(comps, 18, 9, 125, 15, 0, tile::getClockState, (i)-> sendPkt((byte)-1)).texture(168, 148).tooltip("gui.rs_ctr.processor.run");
		this.state = StateEditor.of(comps, tile.getCircuit(), tile.getIOLabels(), 12, this::updateVar, false);
		state.move(0, 24);
